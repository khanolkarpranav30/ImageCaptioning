...
transformers: Used for loading the BLIP and BLIP-2 models and processors.
torch: Required as the backend for the "pt" (PyTorch) tensors generated by the model.
gradio: Provides the web interface for interactive image captioning.
pillow (PIL): Used for opening, converting, and processing image files.
numpy: Handles image data as arrays for the Gradio interface.
pandas & openpyxl: Enables the creation of the final Excel report (Caption_df.xlsx) during batch processing.
